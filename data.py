        ...
        if self.allowed_tokens is None:
            ...
            for index in self.indices.values():
                for i, token in enumerate(index):
                    # print(token, tokenizer(token)["input_ids"])
...
class SeqRecDataset(BaseDataset):
    ...
    def _remap_items(self):
        ...
        for uid, items in self.inters.items():
            ...
            # print(self.remapped_inters[uid])

    def _remap_items_shuffle(self):

        self.remapped_inters = dict()
        for uid, items in self.inters.items():
            new_items = []
            for i in items:
                res = self.indices[str(i)]
                random.shuffle(res)
                new_items.append("".join(self.indices[str(i)]))
            # new_items = ["".join(self.indices[str(i)]) for i in items]
            self.remapped_inters[uid] = new_items
            # print(self.remapped_inters[uid])
    ...
    def _get_text_data(self, data, prompt):
        if self.args.shuffle:
            ...
            item = [a[:-1] for a in item.split('<')[1:]]
            # item = sorted(item, key=lambda x:x[0])
            random.shuffle(item)
            data['item'] = f'<{item[0]}><{item[1]}><{item[2]}><{item[3]}>'
            for i in inters.split(', '):
                inter_item = [a[:-1] for a in i.split('<')[1:]]
                # inter_item = sorted(inter_item, key=lambda x: x[0])
                random.shuffle(inter_item)
                shuffled_inters.append(f'<{inter_item[0]}><{inter_item[1]}><{inter_item[2]}><{inter_item[3]}>')
                final_shuffled_inters = ', '.join(shuffled_inters)
            data['inters'] = final_shuffled_inters
    ...
    def __getitem__(self, index):
        ...
        # print('seqrec')
        return dict(input_ids=input, labels=output, type='seqrec')
...
class ItemFeatDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        return dict(input_ids=input, labels=output, type='itemfeat')
...
class ItemSearchDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        return dict(input_ids=input, labels=output, type='itemsearch')
